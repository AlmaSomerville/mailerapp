...
export async function rememberWaiting(campaign, count) {
  ...
  try {
    await redis(['SET', key(`waiting:${campaign}`), String(count)]);
    await redis(['EXPIRE', key(`waiting:${campaign}`), 7200]);
  } catch { /* best effort */ }
}
...
export async function readWaiting(campaignKeys) {
  if (!URL_ || !TOKEN) return {};
  const out = {};
  await Promise.all((campaignKeys || []).map(async (k) => {
    try {
      const v = await redis(['GET', key(`waiting:${k}`)]);
      if (v !== null && v !== undefined) out[k] = parseInt(v, 10);
    } catch { /* best effort */ }
  }));
  return out;
}
 ...
 */
export async function checkAlerts({ waiting, allocated, sentBy, windowBy, paused = [], tiers = {}, errors = 0 }) {
