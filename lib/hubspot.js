 ...
 *
 * getDueContacts asks a single global "who is due?", ordered by dw_next_send
 * ascending and capped at MAX_PER_RUN. That shared queue is what lets a released
 * backlog take every slot: its timestamps are older, so it sorts first and newer
 * leads never enter the batch at all. One query per campaign removes the shared
 * queue, so no campaign can crowd out another.
 */
...
/**
 * How many contacts are waiting, per campaign, without pulling the records.
 * A search returns `total`, so limit:1 is enough. The cron reports this, so a
 * campaign quietly piling up is visible before anyone notices their leads
 * stopped being contacted.
 */
export async function getDueCounts(campaignKeys) {
  const now = Date.now();
  const counts = {};

  await Promise.all((campaignKeys || []).map(async (key) => {
    try {
      const data = await hs('/crm/v3/objects/contacts/search', {
        method: 'POST',
        body: JSON.stringify({
          filterGroups: [{
            filters: [
              { propertyName: 'dw_campaign', operator: 'EQ', value: key },
              { propertyName: 'dw_next_send', operator: 'LTE', value: String(now) },
              { propertyName: 'hs_email_optout', operator: 'NEQ', value: 'true' }
            ]
          }],
      ...
      counts[key] = null;   // a failed count must never stop a run
    }
  }));
