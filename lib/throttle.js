...
export async function smsGate(line, now = new Date()) {
  if (!URL_ || !TOKEN) return { ok: true };
  try {
    const [lineRaw, globalRaw, dayRaw] = await Promise.all([
      redis(['GET', smsTickKey(line, now)]),
      redis(['GET', smsTickKey('', now)]),
      SMS_DAILY_CAP > 0 ? redis(['GET', smsDayKey(line, now)]) : Promise.resolve('0')
    ]);
