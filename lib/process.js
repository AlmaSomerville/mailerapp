...
async function processContactCore(contact, campaigns, ownerMap) {
  ...
  if (channel === 'sms') {
    ...
    if (!r.ok) {
      ...
      if (kind === 'systemic') {
        ...
        // OpenPhone authorises per phone NUMBER, not per workspace — a 403 here usually
        // means the API key's user isn't a member of this owner's line, which is why it
        // can hit one or two owners while every other number on the same key sends fine.
        return holdOrDrop(r.error, 'OpenPhone credentials, billing, or number membership need attention');
      }
  ...
  if (step.format === 'design' && effectiveStep.designId) {
    ...
    if (!record?.html) {
      await logEvent({ type: 'error', ...base, detail: `design "${effectiveStep.designId}" missing or has no HTML` });
      return { status: 'error', detail: `design "${effectiveStep.designId}" not found` };
    }
