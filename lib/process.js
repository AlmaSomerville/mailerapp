...
async function processContactCore(contact, campaigns, ownerMap) {
  ...
  if (channel === 'sms') {
    ...
    // Flood protection proper: pace the sends instead of discarding them. Over-cap texts are
    // DEFERRED — dw_next_send is left untouched, so the contact stays due and the next tick
    // picks it up. Same mechanism as the send-window gate above, so nothing here can strand
    // a contact mid-sequence. If Quo goes down, sends fail, contacts stay due, and this meters
    // the recovery on its own.
    const smsPace = await smsGate();
    if (!smsPace.ok) {
      return { status: 'skipped', detail: `deferred — ${smsPace.reason}` };
    }
